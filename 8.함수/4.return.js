...
function add(n1, n2){
    return n1 + n2;//반복문의 break처럼 return을 만나면 코드 중지 
}

function showMultiply(n1, n2){
    console.log(`${n1} X ${n2} = ${n1 * n2}`);
    //결과값만 보고 싶다면 return을 쓰지 않아도 됨 
}
...
//return이 없는 void함수
function greeting (nikname) {
    if(nikname === '바보'){
        console.log('그런 나쁜말은 쓰지 마세요.');
        return;
    }
    console.log(`${nikname}님 안녕하세요.`);
}
//return이 없는 void함수는 변수에 저장 금지(undefined 저장됨)
//오로지 단독 호출 
greeting('김철수');
greeting('김영희');
...
// 2개의 정수를 전달받아서 덧셈, 뺄셈, 곱셈, 나눗셈 결과를 다 리턴하고 싶다.
